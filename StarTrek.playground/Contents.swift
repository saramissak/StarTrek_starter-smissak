...
var numHumanOfficers = humanOfficers.count


// Q1: What percentage of humans are female?
var percentFemale = humanOfficers.filter{ femaleOfficers.contains($0)}.count
var percent = Double(percentFemale) / Double(numHumanOfficers)
print(percent)
...
let allCaptians = captains.map{ Officer(name: $0, ship: $1)}
print(allCaptians.map{$0.name})



// Q5: Alphabetize the previous list by the captain's name
let sorted = allCaptians.sorted()
/*:
Group 2
*/
// Q6: Create an array of officers serving as first officers who are also Vulcan
let firstOfficerObj = firstOfficers.map{ Officer(name: $0, ship: $1)}
let vulcanFirstOfficers = firstOfficerObj.filter{ vulcanOfficers.contains($0.name) }
print(vulcanFirstOfficers)


// Q7: How many these Vulcan first officers have names shorter than 5 characters?
let q7 = vulcanFirstOfficers.filter{ $0.name.count < 5 }.count
print(q7)
